...
namespace Sistema.Presentacion
{
    public partial class ConsultaRecibo : Form
    {
        ...
        private void btn_devolver_Click(object sender, EventArgs e)
        {
            ...
            bool devoluciones_ = false;


            if (Dgv_cRecibos.RowCount > 0)
            {

                     //devo_ = Convert.ToInt32(Dgv_cRecibos.Rows[i].Cells[15].Value);
            ...
            if (id_.CompareTo("") == 0 || motivo_.CompareTo("") == 0 || codigo_.CompareTo("") == 0)
            {
            ...
            else
            {
                ...
                devoluciones_ = bool.Parse(devolucion.Text);
                if (devoluciones_ == false)
                {
                    MessageBox.Show("Departamento sin devoluciones", "Informacion", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
        ...
        private void btn_devolvertodo_Click(object sender, EventArgs e)
        {
            ...
            if (Dgv_cRecibos.RowCount > 0)
            {
                for (int i = 0; i < Dgv_cRecibos.RowCount; i++)
                {
                    devo_ = Convert.ToBoolean(Dgv_cRecibos.Rows[i].Cells[15].Value);
                }
            }
        ...
        private void Dgv_cRecibos_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else
            {
                ...
                subtotal.Text = Dgv_cRecibos.CurrentRow.Cells[12].Value.ToString();
                devolucion.Text = Dgv_cRecibos.CurrentRow.Cells[15].Value.ToString();
            }
        }
        ...
        private void btn_recibo_Click(object sender, EventArgs e)
        {
            ...
            if (Dgv_cRecibos.RowCount < 1)
            {
                MessageBox.Show("No hay recibo seleccionado", "Informacion", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            else
            {
                ...
                for (int i = 0; i < int.Parse(Numero_Pedidos); i++)
                {
                    ...
                    string desc = Convert.ToString(table.Rows[i][11]);
                    string importe = Convert.ToString(table.Rows[i][12]);
        ...
        private void cb_numRecibos_KeyPress(object sender, KeyPressEventArgs e)
        {
